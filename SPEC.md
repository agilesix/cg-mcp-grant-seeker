...
- Existing data path: MCP tools → CommonGrants SDK client boundary → federal, Pennsylvania, California, and Washington APIs.
...
- Built-in federal, Pennsylvania, California, and Washington sources use standalone local SDK consumer plugins.
...
- Use host-provided layout context and light/dark color scheme independently from the configured
  product visual theme.
- Keep the complete visual theme in `src/views/theme.css`. View component CSS owns layout and
  responsive behavior but contains no palette, typeface, type scale, radius, elevation, focus, or
  state-color decisions. Self-hosters can restyle the app by replacing semantic token values in
  that one file.
- Default to the CommonGrants-derived visual preset. Allow self-hosters to select `host-neutral`
  through the typed `VITE_GRANT_VISUAL_THEME` build setting. Do not add a user-facing theme picker
  or expose visual configuration through the MCP tool contract.
...
- CommonGrants is the default visual preset; a host-neutral build preserves the same view behavior.
- The visual treatment can be changed through `src/views/theme.css` without modifying the React
  view or its structural CSS.
